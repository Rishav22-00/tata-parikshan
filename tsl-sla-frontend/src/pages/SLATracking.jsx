...
import { Container, Card, Table, Form, Button, Row, Col, Badge, Alert, Spinner } from 'react-bootstrap';
import { useParams } from 'react-router-dom';
import { useUser } from '../contexts/UserContext';
import { getSLA, saveProgress, getProgress, addComment, getComments } from '../services/api';
...
const SLATracking = () => {
  ...
  const allProgress = months.map(month => {
    ...
    const existing = progress.find(p => 
      new Date(p.month).toISOString().substring(0, 7) === monthStr
    );
    ...
    <Container className="mt-4">
      <Card className="mb-4">
        <Card.Header>
          ...
          <div>
            <Badge bg="success" className="me-2">{sla.status}</Badge>
            <span className="text-muted">Between {sla.raisingDept} and {sla.targetDept} departments</span>
          </div>
        </Card.Header>
        <Card.Body>
            ...
            <tbody>
              {allProgress.map((progressItem) => (
                <tr key={progressItem._id}>
                  <td>{new Date(progressItem.month).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</td>
                  <td>
                    <ul className="small">
                      {progressItem.updates.map((metric, mIndex) => (
                        <li key={mIndex}>
                          <strong>{metric.metric}:</strong> Target {metric.target}
                        </li>
                      ))}
                    </ul>
                  </td>
                  <td>
                    {progressItem.updates.map((metric, mIndex) => (
                      <Form.Control
                        ...
                        className="mb-2"
                        value={metric.actual}
                        onChange={(e) => handleActualChange(progressItem._id, mIndex, e.target.value)}
                  ...
                  <td>
                    {progressItem.updates.map((metric, mIndex) => (
                      <Form.Select
                        ...
                        className="mb-2"
                        value={metric.status}
                        onChange={(e) => handleStatusChange(progressItem._id, mIndex, e.target.value)}
                      >
                  ...
                  <td>
                    <Form.Control
                      ...
                      onChange={(e) => handleCommentChange(progressItem._id, e.target.value)}
                    />
                  </td>
                  <td>
                    <Button 
            ...
            {comments.length > 0 ? comments.map((comment) => (
              <div key={comment._id} className="d-flex mb-3">
                <div className="me-3">
                  ...
                  >
                    {comment.user?.department?.substring(0, 2).toUpperCase() || '??'}
                  </div>
                </div>
                <div className="flex-grow-1">
                  ...
                  {comment.progress && (
                    <div className="text-muted small mb-1">
                      Regarding: {new Date(comment.progress.month).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                    </div>
                  )}
